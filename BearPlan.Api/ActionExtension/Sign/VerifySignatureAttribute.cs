...
namespace BearPlan.Api.ActionExtension.Sign;
...
public class VerifySignatureAttribute : BaseActionFilter
{
    ...
    public override async Task OnActionExecuting(ActionExecutingContext filterContext)
    {
        ...
        {
            filterContext.Result = Error(string.Format(Language.Arg0_required, "appId"));
            return;
        }
        ...
        {
            filterContext.Result = Error(string.Format(Language.Arg0_required, "time"));
            return;
        }
        ...
        {
            filterContext.Result = Error(string.Format(Language.Arg0_required, "guid"));
            return;
        }
        ...
        {
            filterContext.Result = Error(Language.Error_DuplicateCallNotAllowed);
            return;
        }
        ...
        {
            filterContext.Result = Error(string.Format(Language.Arg0_required, "sign"));
            return;
        }

        var appSecretModel = await filterContext.HttpContext.RequestServices.GetRequiredService<IAppSecretService>()
            .GetIQueryable(x => x.AppId == appId).FirstAsync();
        if (appSecretModel.IsNull())
        {
            UpdateAppSecretParam param = new UpdateAppSecretParam
            {
                AppId = appId
            };
            filterContext.Result =
                Error(ValidationError.NotExist(param, nameof(Language.Sys_AppSecret),
                    nameof(param.AppId)));
            return;
        }
