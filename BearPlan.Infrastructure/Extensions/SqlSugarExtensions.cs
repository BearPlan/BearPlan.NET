...
using BearPlan.Core;
using BearPlan.Core.Caches.SqlSugar;
using BearPlan.Core.ConfigOptions;
using BearPlan.Core.Serilog;
...
public static class SqlSugarExtensions
{
    ...
    public static void AddSqlSugarSetup(this IServiceCollection services)
    {
        ...
        {
            throw new Exception(Language.Error_AppSettings_DataConnection);
        }
        ...
        {
            throw new Exception(string.Format(Language.Error_Database_EnsureEnabled_Arg0,
                systemOptions.MainDataBase));
        }

        if (allConnectionItem.All(x => x.ConnId != systemOptions.LogDataBase))
        {
            throw new Exception(string.Format(Language.Error_LogDatabase_EnsureEnabled_Arg0,
                systemOptions.LogDataBase));
        }

        if (allConnectionItem.All(x => x.ConnId != systemOptions.ConfigProviderDataBase))
        {
            throw new Exception(string.Format(Language.Error_ConfigProviderDatabase_EnsureEnabled_Arg0,
                systemOptions.ConfigProviderDataBase));
        }
        ...
        foreach (var connectionItem in allConnectionItem)
        {
            ...
            if (systemOptions.IsCqrs)
            {
                ...
                {
                    throw new Exception(string.Format(Language.Error_Database_SlaveEnsureEnabled_Arg0,
        ...
        var sugar = new SqlSugarScope(allConnectionConfig,
            db =>
            {
                // 仅排除日志库：审计日志为分表，且需记录所有用户操作，不应套用软删除/数据权限过滤器。
                // 配置库（ConfigProviderDataBase）属于业务库，必须保留 DataExecuting（自动赋 Id/时间戳）与过滤器。
    ...
    private static void DataExecuting(object value, DataFilterModel entityInfo)
    {
        ...
        if (entityInfo.EntityValue is BaseEntityNoDataScope<long> baseEntityNoDataScope)
        {
            switch (entityInfo.OperationType)
            {
                case DataFilterType.InsertByObject:
        ...
        SugarParameter[] pars, string configId)
    {
        try
        {
            ...
            var httpUser = App.GetService<IHttpUser>();
            var sqlMsg =
                $"执行DB--> 操作用户:[{httpUser.Account}] 操作类型:[{operate}] 数据库ID:[{configId}] {UtilMethods.GetNativeSql(sql, pars)}";
        ...
        SugarParameter[] pars, string configId)
    {
        try
        {
            ...
            var httpUser = App.GetService<IHttpUser>();
            var sqlMsg =
                $"执行DB--> 操作用户:[{httpUser.Account}] 操作类型:[{operate}] 数据库ID:[{configId}] {UtilMethods.GetNativeSql(sql, pars)}[耗时]:{ado.SqlExecutionTime.TotalMilliseconds}ms";
            if (systemOptions.RunMode == RunMode.Dev && middlewareOptions.MiniProfiler)
            {
                MiniProfiler.Current.CustomTiming("SQL", sqlMsg + "\r\n");
            }
