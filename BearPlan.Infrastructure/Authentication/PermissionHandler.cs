        ...
        if (httpContext != null)
        {
            ...
            if (defaultAuthenticate != null)
            {
                ...
                if (result is { Principal: not null })
                {
                    ...
                    {
                        var tokenMd5 = App.GetService<IHttpUser>().JwtToken.ToMd5String16();
                        var tokenBlacklist = await _tokenBlacklistService.GetIQueryable(x => x.AccessToken == tokenMd5)
                            .FirstAsync();
                        ...
                        var netUser = await _userService.GetInfoAsync(App.GetService<IHttpUser>().Id);
                        if (netUser.IsNull())
                        {
                            context.Fail();
                            return;
                        ...
                        var onlineUser = new LoginUserInfo
                        {
                            ...
                            AccessToken = App.GetService<IHttpUser>().JwtToken
                        };
                    ...
                    var urlAccessControlList = await _permissionService.GetUrlAccessControlAsync(App.GetService<IHttpUser>().Id);
                        ...
                        try
                        {
                            ...
                            if (context.Resource.IsNotNull())
                            {
                                ...
                                if (endpointFeature.Endpoint?.Metadata.FirstOrDefault(x =>
                                        ...
                                        HasRoleAttribute
                                        authorize && authorize.AuthCodes.Any(roleCodes.Contains))
                                {
                                    isMatchRole = true;
                                }
                            }
                    ...
                    if (!isMatchRole)
                    {
                        ...
                        {
                            var authCodes =
                                await _permissionService.GetAuthCodeAsync(App.GetService<IHttpUser>().Id);
                            if (context.Resource.IsNotNull())
                            {
                                var endpointFeature = (IEndpointFeature)((DefaultHttpContext)context.Resource)
                                    .Features.FirstOrDefault(x =>
                                ...
                                if (endpointFeature.Endpoint?.Metadata.FirstOrDefault(x =>
                                        ...
                                        HasPermAttribute
                                        authorize && authorize.AuthCodes.Any(authCodes.Contains))
                                {
                                    isMatchRole = true;
                                }
                            }
                                ...
                                if (endpointFeature != null)
                                {
                                    var online =
                                        endpointFeature.Endpoint?.Metadata.FirstOrDefault(x =>
                                                x.GetType() == typeof(OnlineAttribute)) as
                                            OnlineAttribute;
                                    if (online.IsNotNull())
                                    {
                                        context.Succeed(requirement);
