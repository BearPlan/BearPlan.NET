...
using BearPlan.Core.ClassLibrary;
...
namespace BearPlan.Infrastructure.SeedData
{
    /// <summary>
    ///  种子数据
    /// </summary>
    public class SeedService
    {
        /// <summary>
        /// 数据库初始化状态（供启动面板读取展示）。
        /// 在各 InitXxxDataAsync 成功后置 true；未执行初始化时为 false。
        /// </summary>
        public static bool MasterDbReady { get; private set; }
        public static bool LogDbReady { get; private set; }
        public static bool ConfigProviderDbReady { get; private set; }
        public static bool TenantDbReady { get; private set; }
            ...
            try
            {
                ...
                var entityList = GlobalType.EntityTypes
                    .Where(x => x.GetCustomAttribute<SugarTable>() != null &&
                                ...
                                x.GetCustomAttribute<ConfigProviderDataBaseAttribute>() == null &&
        ...
        /// </summary>
        /// <param name="dataContext"></param>
        /// <param name="configProviderDataBase"></param>
        /// <exception cref="Exception"></exception>
        public static Task InitConfigProviderDataAsync(DataContext dataContext, string configProviderDataBase)
        {
            if (!dataContext.Db.IsAnyConnection(configProviderDataBase))
            {
                throw new ApplicationException("未配置配置数据库，请检查DataConnection节点配置");
            }


            var configProviderDb = dataContext.Db.GetConnectionScope(configProviderDataBase);
            if (configProviderDb.CurrentConnectionConfig.DbType != DbType.Oracle)
            {
                configProviderDb.DbMaintenance.CreateDatabase();
            }
            else
            {
                throw new Exception("sqlSugar不支持Oracle使用代码建库,请先建库后注释该代码重新启动！");
            ...
            var configProviderEntityList = GlobalType.EntityTypes
                .Where(x => x.GetCustomAttribute<SugarTable>() != null &&
                            x.GetCustomAttribute<ConfigProviderDataBaseAttribute>() != null).ToList();


            var configProviderTables = configProviderDb.DbMaintenance.GetTableInfoList();

            configProviderEntityList.ForEach(entity =>
            {
                var entityInfo = dataContext.Db.EntityMaintenance.GetEntityInfo(entity);

                if (!configProviderTables.Any(x => x.Name.Equals(entityInfo.DbTableName, StringComparison.OrdinalIgnoreCase)))
                {
                    if (entity.GetCustomAttribute<SplitTableAttribute>() != null)
                    {
                        configProviderDb.CodeFirst.SplitTables().InitTables(entity);
                    }
                    else
                    {
                        configProviderDb.CodeFirst.InitTables(entity);
                    }
                }
            });
            ConfigProviderDbReady = true;
            return Task.FromResult(Task.CompletedTask);
        }
